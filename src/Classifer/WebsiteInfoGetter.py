...
def getWebSiteText(startYear,startMonth, startDay,endYear,endMonth,endDay,pageSize,docNumber,billType):
...
def getWebSiteDocCount(startYear,startMonth, startDay,endYear,endMonth,endDay,pageSize,billType):
    #Government API URL
    url = 'https://api.govinfo.gov/collections/BILLS/'+startYear+'-'+startMonth+'-'+startDay+'T20%3A18%3A10Z/'+endYear+'-'+endMonth+'-'+endDay+'T20%3A18%3A10Z?offset=0&pageSize='+pageSize+'&docClass='+billType+'&api_key=A5IBMbRtjbh9c7EYatrZmJNzFxkVgU8AGmtk8XbH'
    #Store URL request
    requestPackage = requests.get(url)
    #Store json into J2
